...
std::string ProcessorInput::ProcessConnectRequest(std::string& clientRequest){
    ...
    std::getline(ss, host, ':');
    std::getline(ss, port);
    if(isValidLoginCommand(clientRequest)){
        std::cout << "Error: Invalid login command format." << std::endl;
        return;
    }
    // Further processing can be done here
    std::string serverFrame;
    std::string frameType="CONNECT\n";
    std::string version="accept-version:1.2\n";
    std::string hostHeader="host:stomp.cs.bgu.ac.il\n";
    std::string loginHeader="login:"+login+"\n";
    std::string passcodeHeader="passcode:"+passcode+"\n\n"+'\0';
    serverFrame=frameType+version+hostHeader+loginHeader+passcodeHeader;
